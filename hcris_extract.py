...
Output: hcris_data/hcris_YYYY.csv per year + hcris_data/hcris_combined.csv
...
def open_zip_for_year(year):
    ...
    print(f"  1. Visit: https://www.cms.gov/data-research/statistics-trends-and-reports/cost-reports/cost-reports-fiscal-year")
...
def load_report_file(zf):
    ...
    col_names = [
        ...
        "ADC_NUM", "FI_CREAT_DT", "UTIL_CD", "NPR_DT",
        "SPEC_IND", "FI_RCPT_DT"
    ]

    with zf.open(rpt_file) as f:
        rpt = pd.read_csv(f, header=None, names=col_names,
...
def load_numeric_file(zf):
    ...
    with zf.open(nmrc_file) as f:
        nmrc = pd.read_csv(f, header=None, names=col_names,
                           dtype=str, low_memory=False)
...
def extract_variables(nmrc, rpt, year):
    ...
    hospitals = rpt[["RPT_REC_NUM", "PRVDR_NUM", "NPI",
                      "FY_BGN_DT", "FY_END_DT",
                      "PRVDR_CTRL_TYPE_CD"]].copy()
    ...
    hospitals["cost_per_discharge"] = hospitals["total_operating_costs"] / valid_discharges
...
def diagnose_worksheets(year):
    ...
    for pattern in ["G3", "S3", "E0"]:
        hits = nmrc["WKSHT_CD"][nmrc["WKSHT_CD"].str.contains(pattern, na=False)].unique()
...
#   import pandas as pd
#
#   df = pd.read_csv("hcris_data/hcris_combined.csv")
#
